<?php 
    ...
    $orderclauses=[
        ...
        1=>"SELECT * FROM porducts WHERE category= '11' and instock!='$zero' $orderby price asc",
        2=>"SELECT * FROM porducts WHERE category= '11' and instock!='$zero' $orderby price desc",
        3=>"SELECT * FROM porducts WHERE category= '11' and instock!='$zero' $orderby id desc",
        4=>"SELECT * FROM porducts WHERE category= '11' and instock!='$zero' $orderby rating desc"
    ];
        ...
        while($a<= $num){
            ...
            $products[]=$row;
                ...
                <form method="post" action="" name="proform"  >
                    ...
                    <?php 
                    $id=$row["id"];
                        $vc="SELECT * FROM `views` WHERE productid='$id'";
                        $vcres=mysqli_query($con,$vc);
                        $vccount=mysqli_num_rows($vcres);
                        if($vccount>0){
                            $r=1;
                            while($r<=$vccount){
                                $r++;
                                $vcd=mysqli_fetch_assoc($vcres);
                            }
                        }
                    ?>
                    <input type="hidden" name="viewcount" value="<?= $vcd["viewcount"] ?>">
                </form>
                <?php 
